...
1) **Full-file replacements only.** No partial diffs or “insert between lines.” Every file must be a complete, clean file, paste-over ready.  
...
8) **Never commit secrets.** Store only non-secret identifiers (e.g., projectId, team slug, repo names). Refer to secrets by **name** only (e.g., `VERCEL_TOKEN`, `AIRTABLE_API_KEY`).  
...
- **Imports**  
  Avoid `@/app/../…`; use `@/lib/*`, `@/db/*`, or stable relatives. Root shims ok.

- **Tailwind**  
  Ensure Tailwind/PostCSS/Autoprefixer are dependencies in **both** root and `apps/portal`; `globals.css` has directives and is imported by layout.
...
3) Commit with: `<concise-commit-title>`
...
**Normal run**  
1) Edit `decisions.json` (full file) and commit.  
2) Run the Reconciler workflow.  
3) Open the PR and merge.

**Reset run** (only if “no changes” or generated block is stale)  
1) Replace `playbook.md` with:
   ~~~text
   <!-- CL:START -->
   <!-- CL:END -->
   ~~~
   Commit `playbook: reset generated block`.  
2) Run the Reconciler, open the PR, merge.
...
1) Open the link above.  
2) Paste the file.  
3) Commit: `api: add example route`
